...
// 1. 加载扩展（MV3） 2. 打开真实 1688 商品页 3. content script 提取 4. RAGFlow 智能体分析 5. 截图
...
const RAGFLOW_URL = 'http://114.55.149.192:8090'
const RAGFLOW_KEY = 'ragflow-QSmWWnQG96rLlX-_tpHKT6hKSQ_j-85vyY4s7OMXNTA'
const AGENT_ID = '8563cdb690e611f1b36bf39ef484774d'
...
;(async () => {
  ...
  // 4. 保存 RAGFlow 配置并调用智能体（模拟 popup 的 ANALYZE_1688 消息路径：
  // worker 内直接调用与 service-worker.js 相同的 fetch 逻辑；popup→worker 的消息路径另测）
  await worker.evaluate(async ({ url, key }) => {
    await chrome.storage.local.set({ ragflowUrl: url, ragflowKey: key })
  }, { url: RAGFLOW_URL, key: RAGFLOW_KEY })

  const t0 = Date.now()
  const analyzeResult = await worker.evaluate(async ({ url, key, agentId, prompt }) => {
    const response = await fetch(`${url}/api/v1/agents/chat/completions`, {
      method: 'POST',
  ...
  }, { url: RAGFLOW_URL, key: RAGFLOW_KEY, agentId: AGENT_ID, prompt: extractResult.prompt })
