...
const report = { started_at: new Date().toISOString(), base_url: BASE, checks: [] }
...
for (const app of APPS) {
  ...
  const ok = res.ok && (body?.data?.content || body?.choices?.[0]?.message?.content)
  chatResults.push({ label: app.label, http: res.status, ok: !!ok, elapsed_ms: elapsed, content_len: (body?.data?.content || body?.choices?.[0]?.message?.content || '').length })
}
...
// ─── 8. KB CRUD 仍 blocked（确认降级方案 B 必要性；MaxKB 模式：HTTP 200 + body.code:500 "方法不被允许"）────────────
...
const realSource = {
  ...
  async fetchDocContent(kbId, docId) {
    ...
    const body = await res.json()
    return body?.data?.name || ''  // v2.10.5-lts 暂不暴露 paragraph 全文；降级用 name + meta
  }
}
let graphOk = false, graphDetail = {}
try {
  // 动态 import（避免顶部 electron 依赖）
...
let publishOk = false, publishDetail = {}
try {
  for (const app of APPS) {
