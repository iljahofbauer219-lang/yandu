...
def analyze_text(
    ...
    *,
    ...
    """Distill a local transcript/report when the API cannot ingest video URLs."""
    if not source_text.strip():
        raise ValueError("source_text is required")
    prompt = prompt_text if prompt_text else _load_prompt(prompt_path)
    if extra_instructions:
        prompt += "\n\n" + extra_instructions
    prompt += "\n\n## Source video report\n\n" + source_text
    response = _call_with_retry(
        _make_client(api_key), model, [types.Part(text=prompt)], max_retries=1
    )
    return response.text
