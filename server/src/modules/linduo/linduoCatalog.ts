...
// 1. 本文件与 src/shared/linduoCatalog.ts 的 id 集合必须完全一致；
// 2. pricing-fallback.ts 的每个 modelId 必须在目录中存在；
// 3. src/main/advisor/MultimodalVision.ts 引用的 gpt-* id 必须在目录中存在；
// 4. 修改时务必同步两边，并跑 pnpm lint:catalog。
...
export interface LinduoModelEntry {
  ...
  description: string
  /** 选型口诀：1 句话告诉用户"这个模型在哪个场景下用 / 不要用"（渲染层紫色 chip） */
  briefRating: string
  contextLabel?: string
...
/** 过滤出可用于聊天的模型。chat-models-sync.ts 启动同步时调用。*/
