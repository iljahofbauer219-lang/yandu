...
exports.ebayResearchQuerySuggestion = ebayResearchQuerySuggestion;
exports.ebayResearchQuerySingular = ebayResearchQuerySingular;
...
function ebayResearchQuerySingular(word) {
    if (word.length <= 4)
        return word;
    if (/ves$/.test(word))
    ...
    if (/s$/.test(word) && !/us$|is$/.test(word))
        return word.slice(0, -1);
    return word;
}
function ebayResearchQuerySuggestion(listing) {
    const specifics = listing.itemSpecifics || [];
    const specificValue = (pattern) => { const value = specifics.find(item => pattern.test(item.name.trim()))?.value.replace(/\s+/g, ' ').trim() || ''; return ebayResearchQueryInvalidSpecificValue.test(value) ? '' : value; };
    const categoryAnchor = (listing.categoryName.split(/\s*>+\s*/).pop() || listing.categoryName).trim();
    const fallback = { query: categoryAnchor.slice(0, 120), source: 'CATEGORY' };
    ...
    const isDimensionToken = (token) => /\d/.test(token) && (/^\d+(?:\.\d+)?x\d/i.test(token) || /^[a-z]?\d+(?:\.\d+)?(?:["”']|in(?:ch(?:es)?)?\.?|cm|mm|ft|feet|foot|pcs?|packs?|ml|cl|l|oz|lb|kg|g|mah)?$/i.test(token));
    const tokens = original.split(' ').map(raw => raw.replace(/^[\s"'“”‘’()[\]{}.,;:!?|/\\&-]+|[\s"'“”‘’()[\]{}.,;:!?|/\\&-]+$/g, '')).filter(raw => {
        const lower = raw.toLowerCase();
        if (!/[a-z0-9]/i.test(raw))
            return false;
        if (ebayResearchQueryDropWords.has(lower) || brandTokens.has(lower) || isDimensionToken(raw))
            return false;
        if (raw.includes('-') && raw.split('-').every(part => !part || ebayResearchQueryDropWords.has(part.toLowerCase())))
            return false;
        return true;
    });
    if (tokens.length < 2)
    ...
    if (anchorTokens.length) {
        searchAnchor: for (let start = 0; start + anchorTokens.length <= lowered.length; start += 1) {
            for (let offset = 0; offset < anchorTokens.length; offset += 1)
                if (lowered[start + offset] !== anchorTokens[offset])
                    continue searchAnchor;
            anchorStart = start;
            break;
        }
    }
    ...
    if (queryTokens.length < 2) {
        const extra = tokens.find(token => !queryTokens.includes(token));
        if (extra)
            queryTokens = [...queryTokens, extra];
    }
