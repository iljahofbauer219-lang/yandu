...
exports.buildEbayMarketStats = buildEbayMarketStats;
exports.normalizeEbayResearchToken = normalizeEbayResearchToken;
...
function normalizeEbayResearchToken(token) {
    if (token.length <= 4)
        return token;
    if (/(?:ch|sh|x)es$/.test(token))
        return token.slice(0, -2);
    if (/ies$/.test(token))
        return `${token.slice(0, -3)}y`;
    if (/[bcdfgklmnprtvz]es$/.test(token))
        return token.slice(0, -1);
    if (/[bcdfghjklmnpqrtvwxz]s$/.test(token))
        return token.slice(0, -1);
    return token;
}
function buildEbayMarketStats(samples, factText, size) {
    ...
    for (const sample of samples) {
        ...
        const terms = new Map();
        if (size === 1)
            entries.forEach(entry => terms.set(entry.token, Boolean(terms.get(entry.token)) || entry.early));
        else
            for (let index = 0; index < entries.length - 1; index += 1) {
                const term = `${entries[index].token} ${entries[index + 1].token}`;
                terms.set(term, Boolean(terms.get(term)) || entries[index].early);
            }
    ...
    return [...counts.entries()].sort((a, b) => (scores.get(b[0]) || 0) - (scores.get(a[0]) || 0) || b[1] - a[1] || a[0].localeCompare(b[0])).slice(0, size === 1 ? 20 : 12).map(([term, count]) => {
        const factStatus = forbidden.test(term) ? 'EXCLUDED' : term.split(' ').every(token => facts.has(token)) ? 'CONFIRMED' : 'REVIEW';
