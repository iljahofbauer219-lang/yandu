...
function classifyAmazonSamples(intent, samples) {
    ...
    const classified = [...unique.values()].map(sample => ({ ...sample, ...classifyAmazonSample(intent, sample) }));
    ...
    return {
        ...
        audit: {
            ...
            confidence: directCount >= 30 ? '可决策' : directCount >= 15 ? '中等' : '低'
        }
    };
}
...
function buildComparableMarketFactBlock(intent, plan, samples, audit) {
    ...
    const lines = [
        ...
        `- 样本审计：原始 ${audit.rawCount}｜ASIN去重 ${audit.uniqueCount}｜DIRECT直接竞品 ${audit.directCount}｜ADJACENT替代方案 ${audit.adjacentCount}｜NON_COMPARABLE排除 ${audit.excludedCount}｜结论置信度：${audit.confidence}`
    ];
    ...
    if (prices.length) {
        const sorted = [...prices].sort((a, b) => a - b);
        const mean = prices.reduce((a, b) => a + b, 0) / prices.length;
        lines.push(`- DIRECT 零售价格区间：${money(sorted[0])}–${money(sorted[sorted.length - 1])}｜中位价 ${money(median(prices))}｜均价 ${money(mean)}`);
    }
    if (ratings.length)
        lines.push(`- DIRECT 评分均值：${round1(ratings.reduce((a, b) => a + b, 0) / ratings.length)}（${ratings.length} 个样本）`);
    if (reviews.length)
        lines.push(`- DIRECT 评论量中位 ${Math.round(median(reviews)).toLocaleString('en-US')}｜均值 ${Math.round(reviews.reduce((a, b) => a + b, 0) / reviews.length).toLocaleString('en-US')}`);
    ...
    lines.push('- NON_COMPARABLE 已从统计和竞品表中排除。所有数值均为系统抓取的搜索页观察值；未抓取的销量、销售额和趋势仍必须写“待验证”。');
    return lines.join('\n');
}
