...
const app = (0, node_fs_1.readFileSync)((0, node_path_1.join)(root, 'src/renderer/App.tsx'), 'utf-8');
// ─── 1. 角色 chips 新阵容 ────────────────────────────────────────
...
assert('岗位由 position 固定', tsx.includes("position = '选品分析师'") && tsx.includes('const agentName = position'));
...
assert('Tab 栏按岗位裁剪', tsx.includes("const showWorkbenchTab = position === 'Listing精造师'") && tsx.includes("const showBrowserTab = position === '选品分析师'"));
...
assert('联动 亚马逊→eBay 站点保留', (0, selectionExtract_1.applyPlatformToRequest)(selectionExtract_1.SELECTION_ANALYSIS_REQUEST, 'eBay') === '请帮我分析这款产品在eBay美国站是否有机会，按方法论文档输出完整评估报告。');
assert('联动 eBay→亚马逊', (0, selectionExtract_1.applyPlatformToRequest)('请帮我分析这款产品在eBay英国站是否有机会', 'Amazon') === '请帮我分析这款产品在亚马逊英国站是否有机会');
assert('联动 Ozon 直接替换', (0, selectionExtract_1.applyPlatformToRequest)(selectionExtract_1.SELECTION_ANALYSIS_REQUEST, 'Ozon').includes('在Ozon美国站'));
assert('联动 无匹配原样返回', (0, selectionExtract_1.applyPlatformToRequest)('自由编辑的文本', 'eBay') === '自由编辑的文本');
...
assert('App 工作区实例传 position', app.includes('<AIEmployee position="选品分析师" onBackToHub='));
