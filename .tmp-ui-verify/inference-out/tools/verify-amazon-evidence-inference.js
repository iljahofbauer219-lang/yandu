...
const service = (0, node_fs_1.readFileSync)((0, node_path_1.join)(root, 'src/main/services/AiEmployeeChatService.ts'), 'utf-8');
const main = (0, node_fs_1.readFileSync)((0, node_path_1.join)(root, 'src/main/main.ts'), 'utf-8');
const preload = (0, node_fs_1.readFileSync)((0, node_path_1.join)(root, 'src/preload/preload.ts'), 'utf-8');
const globalDts = (0, node_fs_1.readFileSync)((0, node_path_1.join)(root, 'src/renderer/global.d.ts'), 'utf-8');
...
assert('service 暴露 inferDifferentiationAndCompliance + AmazonInferenceEvidenceInput/Result 类型', service.includes('async inferDifferentiationAndCompliance(input: AmazonInferenceEvidenceInput): Promise<AmazonInferenceEvidenceResult | null>')
...
assert('主进程注册 ai-employee:infer-evidence IPC 并桥接到 service', main.includes("ipcMain.handle('ai-employee:infer-evidence'") && main.includes('aiEmployeeChatService.inferDifferentiationAndCompliance(input)'));
assert('预加载层暴露 inferEvidence 调用入口', preload.includes("'ai-employee:infer-evidence'") && /inferEvidence\s*:\s*\(input: unknown\)\s*:\s*Promise<unknown>/.test(preload));
assert('renderer global.d.ts 含 inferEvidence 类型签名', globalDts.includes('inferEvidence(input:') && globalDts.includes('Promise<{ differentiation: string; compliance: string; model: string; provider: string } | null>'));
assert('AIEmployee.tsx 阶段 3 末调用 window.desktop.aiEmployee.inferEvidence', tsx.includes('window.desktop.aiEmployee.inferEvidence({') && tsx.includes('listingEvidence,') && tsx.includes('reviewEvidence,'));
// ─── 2. 提示词组装：含身份 + 1688 + Amazon 详情页 bullet + 评论样本；并禁止品牌名/费用数字 ─────────────
const promptHasIdentities = service.includes('产品名称：${input.intent.productName}') && service.includes('形态（人工确认）：${input.sourceText.productForm}');
...
assert('提示词含身份 + 1688 证据 + Amazon 详情页/评论样本 + 严禁品牌/费用 + JSON 输出格式', promptHasIdentities && promptHasSourceText && promptHasAmazonEvidence && promptHasGuards && promptJsonFormat);
// ─── 3. 字符截断 800：JSON 解析后 differentiation/compliance 各截断到 800 字符 ─────────────
assert('结果截断上限 800 字符（INFERENCE_RESULT_MAX_CHARS）', service.includes('const INFERENCE_RESULT_MAX_CHARS = 800') && /trimChars\(parsed\.differentiation,\s*INFERENCE_RESULT_MAX_CHARS\)/.test(service));
assert('bullet points 上限 5 条 × 200 字符 + 评论样本上限 3 条 × 300 字符', service.includes('const INFERENCE_BULLET_LIMIT = 5') && service.includes('const INFERENCE_BULLET_CHARS = 200')
...
assert('parseInferenceJson 移除 Markdown 围栏 + 抽取 {...} 块 + JSON.parse + 字段长度 ≥ 8 校验', /function parseInferenceJson[\s\S]*?replace\(/ `.{0,8}json/.test(service)
    && /function parseInferenceJson[\s\S]*?match\(/\{\[\\s\\S\]\*\}/.test(service)
    && /JSON\.parse\(match\[0\]\)/.test(service)
    && /differentiation\.length < 8 \|\| compliance\.length < 8/.test(service)
)
