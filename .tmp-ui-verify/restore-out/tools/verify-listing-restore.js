...
const listingArchive_1 = require("../src/shared/listingArchive");
const listingBridge_1 = require("../src/shared/listingBridge");
let failures = 0;
...
{
    let archive = [];
    for (let index = 0; index < listingArchive_1.LISTING_ARCHIVE_MAX + 5; index++)
        archive = (0, listingArchive_1.upsertBatch)(archive, makeBatch(`b${index}`));
    assert(`upsert 上限裁剪 ≤ ${listingArchive_1.LISTING_ARCHIVE_MAX}`, archive.length === listingArchive_1.LISTING_ARCHIVE_MAX, `实际 ${archive.length}`);
    assert('upsert 最新置顶', archive[0].id === `b${listingArchive_1.LISTING_ARCHIVE_MAX + 4}`);
    assert('upsert 最旧被裁', !archive.some(item => item.id === 'b0'));
    const replaced = (0, listingArchive_1.upsertBatch)(archive, makeBatch('b25', [{ status: 'done', content: '新内容' }]));
...
{
    const restored = (0, listingArchive_1.normalizeBatchForRestore)(makeBatch('r1', [
    ...
    const summary = (0, listingArchive_1.summarizeBatch)(restored);
    assert('恢复：汇总口径正确', summary.total === 4 && summary.done === 1 && summary.failed === 1 && summary.interrupted === 2);
}
// ─── 3. 字段解析 + CSV：对 P2 真实验收包逐一检验 ──────────────────
{
    const dir = (0, node_path_1.join)(process.env.LISTING_REPO_ROOT || process.cwd(), '.tmp-ui-verify');
    const files = (0, node_fs_1.readdirSync)(dir).filter(name => name.startsWith('listing-matrix-') && name.endsWith('.md'));
    assert('真实包素材存在', files.length >= 9, `${files.length} 个`);
    const packages = files.map(name => {
        const content = (0, node_fs_1.readFileSync)((0, node_path_1.join)(dir, name), 'utf8');
        const fields = (0, listingArchive_1.parseListingFields)(content);
    ...
    assert('CSV 单元格转义引号', (0, listingArchive_1.escapeCsvCell)('a"b') === '"a""b"');
    assert('CSV 单元格逗号/换行包裹', (0, listingArchive_1.escapeCsvCell)('a,b\nc').startsWith('"') && (0, listingArchive_1.escapeCsvCell)('普通') === '普通');
    const csv = (0, listingArchive_1.buildListingCsv)(packages);
...
{
    const extracted = (0, listingBridge_1.formatExtractedAsMaterial)({
        title: '  宠物一键退毛梳  ',
    ...
    assert('提取→素材：空对象不崩溃且声明认证未知', (0, listingBridge_1.formatExtractedAsMaterial)({}).includes('认证：未知'));
    const draftMaterial = (0, listingBridge_1.formatDraftAsMaterial)({
    ...
    const amazonSites = (0, listingBridge_1.siteIdsForMarketplace)('AMAZON');
    ...
    assert('平台映射：EBAY → 2 站点', (0, listingBridge_1.siteIdsForMarketplace)('EBAY').length === 2);
    assert('平台映射：OZON → 空（无对应站点）', (0, listingBridge_1.siteIdsForMarketplace)('OZON').length === 0);
    const languages = (0, listingBridge_1.defaultLanguagesForSites)(amazonSites);
