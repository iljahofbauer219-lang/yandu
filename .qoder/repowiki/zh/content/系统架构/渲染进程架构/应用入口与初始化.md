...
## 更新摘要
**所做更改**   
- 移除了独立的 DeepSeek Harness 路由入口，将其整合到统一的 Codex 体验中
- 更新了 AppPage 联合类型，删除了 `'deepseek-harness'` 类型定义
- 清理了相关的 DeepSeekHarness 组件引用和样式代码
- 增强了 Codex 体验的统一性和完整性
- 简化了图片生成系统，专注于 eBay 为中心的电商功能
...
**最新更新**：应用已完成 DeepSeek Harness 功能的统一化重构，移除了独立的路由入口，将相关能力完全整合到 Codex 体验中，提升了用户体验的一致性和系统的可维护性。同时简化了图片生成系统，专注于 eBay 为中心的电商功能。
...
B --> C["App.tsx<br/>根组件/路由/状态<br/>统一Codex体验"]
D["vite.config.ts<br/>构建与开发服务器配置"] --> A
E["package.json<br/>脚本与依赖声明"] --> D
F["eBay中心化功能<br/>简化的电商工作流"] --> C
```

图表来源
- [src/renderer/index.html:1-14](file://src/renderer/index.html#L1-L14)
- [src/renderer/main.tsx:1-39](file://src/renderer/main.tsx#L1-L39)
- [src/renderer/App.tsx:1-800](file://src/renderer/App.tsx#L1-L800)
...
- App.tsx：根组件，组织路由、全局状态（如用户信息、平台配置、功能开关）、错误边界与日志上报。**已重构以支持统一的Codex体验和简化的图片生成系统**。
...
participant App as "App.tsx<br/>统一Codex体验"
...
```mermaid
graph LR
Main["main.tsx"] --> App["App.tsx<br/>统一Codex体验"]
App --> Router["路由模块"]
App --> Store["状态管理"]
App --> Plugins["插件管理器"]
App --> Services["第三方服务"]
...
- **统一体验优化**：Codex统一界面减少了路由跳转开销，提升了用户体验流畅度。简化的图片生成系统进一步优化了性能表现。
...
- 常见问题
  ...
  - **Codex体验问题**：检查统一路由配置和相关服务集成状态。
  - **eBay功能问题**：检查eBay相关的API连接和业务逻辑。
- 定位手段
  - 控制台日志：开启详细日志级别，过滤关键模块输出。
  - 错误边界：查看错误边界捕获的错误信息与堆栈。
  - 网络面板：检查请求状态码、响应体与跨域问题。
  - 性能面板：分析长任务、重排重绘与内存占用。
  - **统一体验调试**：检查Codex相关的路由和服务连接状态。
  - **eBay功能调试**：检查eBay相关的API调用和业务状态。
- 恢复策略
  - 降级功能：在关键服务失败时回退到本地或离线模式。
  - 重试机制：对网络请求实现指数退避重试。
  - 用户提示：给出明确错误提示与操作指引。
  - **统一体验回滚**：支持切换到备用界面或服务降级方案。
  - **eBay功能回滚**：支持切换到备用的电商功能模块。
...
## 结论
通过对渲染端入口与初始化流程的系统化梳理，明确了 main.tsx 的渲染职责、App.tsx 的根结构与状态管理、以及构建与运行环境的配置要点。结合错误边界、性能优化与调试手段，可有效提升应用的稳定性与可维护性。

**重要更新**：应用已成功完成DeepSeek Harness功能的统一化重构，移除了独立的路由入口和组件，将所有AI能力整合到统一的Codex体验中。同时简化了图片生成系统，专注于eBay为中心的电商功能。这一变更简化了应用架构，提升了用户体验的一致性，并为未来的功能扩展奠定了更好的基础。建议在实际项目中继续完善环境变量管理、插件接口规范与服务容错机制，以支撑复杂业务场景下的稳定运行。

## 附录
- 启动参数与环境变量
  - 常用参数：NODE_ENV、端口号、调试标志、功能开关等。
  - 建议将环境变量集中在 .env 文件中，并按环境区分。
  - **Codex相关环境变量**：CODEX_MODE、CODEX_DEBUG等。
  - **eBay相关环境变量**：EBAY_API_KEY、EBAY_MARKETPLACE等。
- 调试模式设置
  - 开发模式：启用热重载、源码映射、详细日志。
  - 生产模式：关闭调试输出、启用压缩与混淆。
  - **Codex调试模式**：启用统一体验的调试信息和错误追踪。
  - **eBay调试模式**：启用eBay相关功能的详细日志和错误追踪。
- 第三方服务集成清单
  - 翻译服务、图像合规、视频处理等服务的接入方式、鉴权与限流策略。
  - **Codex服务集成**：统一的AI服务接口和认证机制。
  - **eBay服务集成**：eBay API、商品同步、订单管理等服务的集成配置。
- **统一体验配置**
  - Codex界面配置
  - 路由统一策略
  - 服务集成规范
