...
import { BailianModelMallPage } from './BailianModelMallPage'
...
export function App() {
  ...
  // 树外权限码（如 dashboard.view）退化为 hasPermission 直接判断，避免永久不可达
  const canMenu = (code: string) => {
    const node = MENU_PERMISSION_TREE.find(n => n.code === code)
    if (!node) return hasPermission(profile, code)
    return hasMenuAccess(c => hasPermission(profile, c), node)
  }
  ...
  useEffect(() => {
    ...
    const first = aiModuleNav.find(item => canMenu(item.perm))
