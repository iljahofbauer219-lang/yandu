...
export interface AmazonMarketSample {
  asin: string
  title: string
  price: number | null
  rating: number | null
  ...
  page?: number
  sponsored?: boolean
  source?: 'api' | 'browser' | 'hybrid'
  salesVolume?: string | null
  bsr?: number | null
}

/** Amazon 商品详情页可直接读取的事实证据。所有字段均保留来源页面与采集时间，缺失即不填。 */
export interface AmazonListingEvidence {
  asin: string
  url: string
  capturedAt: string
  source: 'browser'
  ...
  seller?: string | null
  operations: string[]
  /** 单件毛重（克）。从 Amazon Product Details 表 Item Weight / Shipping Weight 解析。 */
  itemWeightGrams?: number | null
  /** 包装尺寸（厘米，L×W×H）。从 Package Dimensions / Product Dimensions 解析。 */
  packageDimensionsCm?: { length: number; width: number; height: number } | null
  /** FBA Size Tier 推断：仅基于已抓到的重量+尺寸命中后填写，未命中保留 undefined。 */
  sizeTierGuess?: 'SmallStandard' | 'LargeStandard' | 'LargeBulky' | 'ExtraLarge' | null
}

/** Amazon 评论页可见的原始评论片段，不统计情绪比例或外推全量评论。 */
export interface AmazonReviewEvidence {
  asin: string
  url: string
  capturedAt: string
  source: 'browser'
  snippets: Array<{ rating: number | null; title: string; body: string }>
}

export interface AmazonDataSourceSearchMeta {
  source: 'api'
...
export interface AmazonDataSourceSearchResult {
  samples: AmazonMarketSample[] | null
  error?: string
  meta?: AmazonDataSourceSearchMeta
}

export type AmazonComparisonClass = 'DIRECT' | 'ADJACENT' | 'NON_COMPARABLE'

export interface AmazonSearchIntent {
  productName: string
  productForm: string
  useMethod: string
  targetObject: string
  excludedTerms: string[]
  retailUnit?: AmazonRetailUnit
}

export interface AmazonRetailUnit {
  kind: 'volume_ml' | 'count'
  quantity: number
  label: string
}

/** 快速市场利润率只覆盖可在市场调研阶段直接复算的单位变动成本。 */
export interface AmazonQuickMarketProfitInput {
...
export interface AmazonCostRange {
  low?: number
  base?: number
  high?: number
}

export interface AmazonFullCostInput {
  packagingQcUsd?: AmazonCostRange
  domesticFreightUsd?: AmazonCostRange
  firstLegFreightUsd?: AmazonCostRange
  dutyUsd?: AmazonCostRange
  customsClearanceUsd?: AmazonCostRange
  inboundUsd?: AmazonCostRange
  storageUsd?: AmazonCostRange
}

/** 阶段5入场门禁由用户明确声明的利润目标与可核验依据驱动，不使用系统隐含阈值。 */
export interface AmazonEntryDecisionInput {
  targetContributionMargin?: number
  differentiationEvidence?: string
  complianceIpEvidence?: string
}

/** 阶段 5 仅关心字段是否已能支撑决策，具体来源详情保留在选品提取契约中。 */
export type AmazonProfitDecisionEvidence = Partial<Record<string, { decisionEligible: boolean; origin?: string; note?: string }>>

export type AmazonEntryDecision = '✅ 建议入场' | '⚠️ 有条件谨慎入场' | '❌ 不建议入场' | '❓ 数据不足，不能判定'

export interface AmazonEntryDecisionResult {
...
export interface AmazonKeywordPlan {
  keywords: string[]
  source: 'model' | 'deterministic'
}

export interface ClassifiedAmazonMarketSample extends AmazonMarketSample {
  comparisonClass: AmazonComparisonClass
  comparisonReason: string
}

export interface AmazonSampleAudit {
  rawCount: number
  organicCount: number
  ...
  salesSignalLowerBound: number
  coveragePercent: number
  confidence: '可决策' | '中等' | '低'
}

export interface AmazonSamplingAuditInput {
  keywordsRequested?: number
  keywordsSucceeded?: number
}

/** 达标仅表示可进入市场竞争力评估，不自动等同于“建议入场”。 */
export const AMAZON_RESEARCH_SAMPLE_BASELINE = {
  keywordsRequested: 3,
  keywordsSucceeded: 3,
  directCount: 15,
...
export function meetsAmazonResearchSampleBaseline(audit: AmazonSampleAudit): boolean {
  return audit.keywordsRequested === AMAZON_RESEARCH_SAMPLE_BASELINE.keywordsRequested
    && audit.keywordsSucceeded >= AMAZON_RESEARCH_SAMPLE_BASELINE.keywordsSucceeded
    && audit.directCount >= AMAZON_RESEARCH_SAMPLE_BASELINE.directCount
    && audit.fieldCoveragePercent >= AMAZON_RESEARCH_SAMPLE_BASELINE.fieldCoveragePercent
    && audit.coveragePercent >= AMAZON_RESEARCH_SAMPLE_BASELINE.coveragePercent
}
...
function unitNumber(value: number): string {
  return Number.isInteger(value) ? String(value) : String(Math.round(value * 100) / 100)
}

function retailUnitMultiplier(text: string, unitIndex: number): number {
...
function retailUnitFromText(value: string): AmazonRetailUnit | null {
  ...
  if (multipliedVolume) {
    ...
    if (Number.isFinite(quantity) && quantity > 0) return { kind: 'volume_ml', quantity, label: `${unitNumber(quantity)}ml` }
  }
  ...
  if (count) {
    ...
    if (Number.isFinite(quantity) && quantity > 0) return { kind: 'count', quantity, label: `${unitNumber(quantity)}件` }
  }
  return null
}

function retailUnitFromIdentity(info: SelectionIdentity): AmazonRetailUnit | null {
...
export function buildAmazonSearchIntent(info: SelectionIdentity): AmazonSearchIntent {
...
type AmazonCoreUse = 'CLEANSE' | 'BITE_DETERRENT'

/**
 * 可比性的核心用途只能由已锁定的本品身份推导；检索词和竞品分类共用本规则，
 * 避免把“防咬喷雾”错误套进“洗护喷雾”的固定词表。
 */
function coreUseOf(value: string): AmazonCoreUse | null {
  const text = String(value || '').toLowerCase()
  if (/防咬|防啃|防舔|苦味|苦苹果|苦苹果|anti[ -]?bite|anti[ -]?chew|bitter(?:\s+apple)?|chew\s+deterrent|no[ -]?chew|stop\s+chewing|biting\s+deterrent/.test(text)) return 'BITE_DETERRENT'
  if (/清洁|洗护|洗澡|免洗|除臭|clean|cleans|groom|bath|wash|deodor|shampoo|body\s+cleanser|no[ -]?rinse/.test(text)) return 'CLEANSE'
  return null
}

function intentCoreUse(intent: AmazonSearchIntent): AmazonCoreUse | null {
  return coreUseOf(`${intent.productName} ${intent.useMethod}`)
}

function coreUseMatches(intent: AmazonSearchIntent, value: string): boolean {
  const expected = intentCoreUse(intent)
  return expected != null && coreUseOf(value) === expected
}

/** 无模型或模型输出漂移时仍可按锁定形态生成稳定的三组买家检索词。 */
export function fallbackAmazonKeywords(intent: AmazonSearchIntent): string[] {
  ...
  if (intentCoreUse(intent) === 'BITE_DETERRENT') {
    const byDeterrentForm: Record<string, string[]> = {
...
export function normalizeAmazonKeywordPlan(intent: AmazonSearchIntent, modelKeywords: string[]): AmazonKeywordPlan {
  ...
  for (const raw of [...modelKeywords, ...fallbackAmazonKeywords(intent)]) {
    ...
    if (!keyword || keyword.split(' ').length < 2 || keyword.split(' ').length > 7) continue
    if (excluded.some(term => keyword.includes(term))) continue
    if (!coreUseMatches(intent, keyword)) continue
    if (!accepted.includes(keyword)) accepted.push(keyword)
...
function targetMatches(title: string, targetObject: string): boolean {
  if (/猫(?!狗)|cat/i.test(targetObject) && !/狗|dog/i.test(targetObject)) return /\b(?:cats?|kittens?|pets?)\b/i.test(title)
  if (/狗(?!猫)|dog/i.test(targetObject) && !/猫|cat/i.test(targetObject)) return /\b(?:dogs?|pupp(?:y|ies)|pets?)\b/i.test(title)
  return /\b(?:pets?|dogs?|pupp(?:y|ies)|cats?|kittens?)\b/i.test(title)
}

function classifyAmazonSample(intent: AmazonSearchIntent, sample: AmazonMarketSample): Pick<ClassifiedAmazonMarketSample, 'comparisonClass' | 'comparisonReason'> {
  ...
  if (directPattern?.test(title)) return { comparisonClass: 'DIRECT', comparisonReason: '对象、任务与产品形态一致' }
...
function salesSignalLowerBound(value?: string | null): number | null {
...
export function classifyAmazonSamples(intent: AmazonSearchIntent, samples: AmazonMarketSample[], input: AmazonSamplingAuditInput = {}): { samples: ClassifiedAmazonMarketSample[]; audit: AmazonSampleAudit } {
  const unique = new Map<string, AmazonMarketSample>()
  for (const sample of samples) {
    const key = sample.asin.trim().toUpperCase()
    if (!key) continue
    const previous = unique.get(key)
    // 同一 ASIN 同时出现在赞助位和自然位时保留自然位；纯赞助位不进入竞品统计。
    if (!sample.sponsored && (!previous || previous.sponsored)) unique.set(key, { ...sample, asin: key })
    else if (!previous) unique.set(key, { ...sample, asin: key })
  }
  ...
  const salesSignals = direct.map(item => salesSignalLowerBound(item.salesVolume)).filter((value): value is number => value != null)
  ...
  const confidence: AmazonSampleAudit['confidence'] = keywordsRequested === AMAZON_RESEARCH_SAMPLE_BASELINE.keywordsRequested
    && keywordsSucceeded >= AMAZON_RESEARCH_SAMPLE_BASELINE.keywordsSucceeded
    && directCount >= AMAZON_RESEARCH_SAMPLE_BASELINE.directCount
    && fieldCoveragePercent >= AMAZON_RESEARCH_SAMPLE_BASELINE.fieldCoveragePercent
    && coveragePercent >= AMAZON_RESEARCH_SAMPLE_BASELINE.coveragePercent
    ? '可决策'
...
export function extractAmazonSamples(doc: Document): AmazonMarketSample[] | null {
  if (/captcha|robot check/i.test(doc.title) || doc.querySelector('form[action*="validateCaptcha"]')) return null
  ...
  for (const card of Array.from(doc.querySelectorAll('div[data-component-type="s-search-result"]'))) {
    ...
    if (!asin) continue
    const title = ((card.querySelector('h2') || {}) as Node).textContent?.replace(/\s+/g, ' ').trim() || ''
    const priceText = (((card.querySelector('.a-price .a-offscreen') || {}) as Node).textContent || '').replace(/[^0-9.]/g, '')
    const ratingText = (((card.querySelector('i.a-icon-star span.a-icon-alt') || card.querySelector('i.a-icon-star') || {}) as Node).textContent || '')
    const ratingMatch = ratingText.match(/([\d.]+)/)
    const reviewEl = card.querySelector('a[href*="customerReviews"] span, a[href*="product-reviews"] span')
    const reviewsText = ((reviewEl || {}) as Node).textContent?.replace(/[^0-9]/g, '') || ''
    samples.push({
      asin,
      title,
      ...
      sponsored: /sponsored/i.test(((card.querySelector('[aria-label*="Sponsored"], .s-sponsored-label-info-icon') || {}) as Node).textContent || '') || Boolean(card.querySelector('[aria-label*="Sponsored"], .s-sponsored-label-info-icon')),
      source: 'browser'
    })
    if (samples.length >= 48) break
  }
  return samples
}

/** Amazon Product Details 表格中 “Item Weight / Shipping Weight” → 克。 */
export function parseAmazonItemWeightGrams(value: string): number | null {
  ...
  if (unit === 'oz' || unit === 'ounces') return Math.round(num * 28.3495)
  if (unit === 'lb' || unit === 'pounds') return Math.round(num * 453.592)
  if (unit === 'kg' || unit === 'kilograms') return Math.round(num * 1000)
  return Math.round(num)
}

/** Amazon Product Details 表格中 “Package Dimensions / Product Dimensions” → 厘米。 */
export function parseAmazonPackageDimensionsCm(value: string): { length: number; width: number; height: number } | null {
  if (!value) return null
  ...
  const toCm = (n: number) => (useCm ? n : n * 2.54)
  return {
...
export function determineAmazonSizeTier(weightGrams?: number | null, dimensionsCm?: { length: number; width: number; height: number } | null): 'SmallStandard' | 'LargeStandard' | 'LargeBulky' | 'ExtraLarge' | null {
  if (!Number.isFinite(weightGrams as number) || (weightGrams as number) <= 0) return null
  const wG = weightGrams as number
  ...
  if (wOz <= 6 && lengthIn <= 4 && widthIn <= 6 && heightIn <= 0.5) return 'SmallStandard'
  ...
  if (wLb <= 20 && withinLargeStandardDims) return 'LargeStandard'
  if (wLb > 70 && wLb <= 150) return 'ExtraLarge'
  if (wLb > 1 && wLb <= 70 && lengthIn <= 108 && widthIn <= 75 && heightIn <= 63) return 'LargeBulky'
  if (wLb > 150) return 'ExtraLarge'
  return null
}

/** 主进程隐藏视图 executeJavaScript 注入串：序列化自 extractAmazonSamples，单一事实源 */
export const AMAZON_SAMPLES_SCRIPT = `(${extractAmazonSamples.toString()})(document)`

/**
 * 详情页解析只读取 Amazon 当前可见 DOM，不推算销量、不补造卖点。
 * 在主进程隐藏视图中执行，因此返回的时间和 URL 是本次抓取的可追溯证据。
 */
function extractAmazonListingEvidence(doc: Document): AmazonListingEvidence | null {
  if (/captcha|robot check/i.test(doc.title) || doc.querySelector('form[action*="validateCaptcha"]')) return null
  ...
  const asin = clean((doc.querySelector('#ASIN') as HTMLInputElement | null)?.value)
...
export interface EstimateFbaFulfillmentFeeInput {
  ...
  sizeTier?: 'SmallStandard' | 'LargeStandard' | 'LargeBulky' | 'ExtraLarge' | null
  /** 售价用于推算 LargeStandard 1–2 lb 的体积重量附加，仅作为分区选择，不参与计算结果。 */
  priceUsd?: number | null
}

export interface EstimateFbaFulfillmentFeeResult {
  /** 推算金额（USD），null 表示无法推算（缺重量或尺寸不匹配 tier）。 */
  feeUsd: number | null
  /** 推算所用 size tier；与 sizeTierGuess 一致。 */
  sizeTier: 'SmallStandard' | 'LargeStandard' | 'LargeBulky' | 'ExtraLarge' | null
  /** 推算描述，用于阶段 4 填写 profitFieldMeta 的 source 字段。 */
  source: string
  /** 警告：推算使用 Amazon US 2024-09 生效费率表；如类目不同（如 Apparel、危险品），需人工补差。 */
  warnings: string[]
}
 ...
 */
export function estimateFbaFulfillmentFee(input: EstimateFbaFulfillmentFeeInput): EstimateFbaFulfillmentFeeResult {
  const warnings: string[] = []
  if (!Number.isFinite(input.weightGrams as number) || (input.weightGrams as number) <= 0) {
    return { feeUsd: null, sizeTier: null, source: '缺少毛重', warnings: ['未抓取到 Item Weight，无法推算 FBA 履约费'] }
  }
  const wG = input.weightGrams as number
  const wOz = wG / 28.3495
  const wLb = wG / 453.592
  ...
  if (tier === 'SmallStandard') {
    if (wOz <= 4) return { feeUsd: 3.06, sizeTier: tier, source: `${FBA_FEE_RATE_TABLE_VERSION}｜SmallStandard ≤ 4 oz = $3.06`, warnings }
    if (wOz <= 6) return { feeUsd: 3.15, sizeTier: tier, source: `${FBA_FEE_RATE_TABLE_VERSION}｜SmallStandard 4–6 oz = $3.15`, warnings }
  ...
  if (tier === 'LargeStandard') {
    if (wLb <= 0.5) return { feeUsd: 3.43, sizeTier: tier, source: `${FBA_FEE_RATE_TABLE_VERSION}｜LargeStandard ≤ 0.5 lb = $3.43`, warnings }
    if (wLb <= 1) return { feeUsd: 3.78, sizeTier: tier, source: `${FBA_FEE_RATE_TABLE_VERSION}｜LargeStandard 0.5–1 lb = $3.78`, warnings }
    if (wLb <= 1.5) return { feeUsd: 4.39, sizeTier: tier, source: `${FBA_FEE_RATE_TABLE_VERSION}｜LargeStandard 1–1.5 lb = $4.39`, warnings }
    if (wLb <= 2) {
    ...
    if (wLb <= 3) return { feeUsd: 5.40, sizeTier: tier, source: `${FBA_FEE_RATE_TABLE_VERSION}｜LargeStandard 2–3 lb = $5.40`, warnings }
    if (wLb <= 20) return { feeUsd: 5.40 + Math.ceil((wLb - 3) / 0.5) * 0.20, sizeTier: tier, source: `${FBA_FEE_RATE_TABLE_VERSION}｜LargeStandard 3–20 lb = $5.40 + 阶梯`, warnings: ['3+ lb 使用阶梯式附加费率，已按 0.5 lb 阶梯粗估，最终金额须用 Amazon Revenue Calculator 复核'] }
    return { feeUsd: null, sizeTier: null, source: 'LargeStandard 重量 > 20 lb', warnings: ['>20 lb 实际为 Large Bulky/Extra Large，需人工按报价补差'] }
  }
  if (tier === 'LargeBulky') {
    if (wLb <= 50) return { feeUsd: 9.61 + Math.ceil((wLb - 1) / 0.5) * 0.42, sizeTier: tier, source: `${FBA_FEE_RATE_TABLE_VERSION}｜LargeBulky 1–50 lb = $9.61 + 阶梯`, warnings: ['LargeBulky 按 0.5 lb 阶梯粗估，最终金额必须以 Amazon Revenue Calculator 输出为准'] }
...
function extractAmazonReviewEvidence(doc: Document): AmazonReviewEvidence | null {
  if (/captcha|robot check/i.test(doc.title) || doc.querySelector('form[action*="validateCaptcha"]')) return null
  const clean = (value: string | null | undefined) => (value || '').replace(/\s+/g, ' ').trim()
...
export const AMAZON_REVIEW_EVIDENCE_SCRIPT = `(${extractAmazonReviewEvidence.toString()})(document)`
...
function normalizedDirectPrices(intent: AmazonSearchIntent, samples: ClassifiedAmazonMarketSample[]): { prices: number[]; excluded: number } {
  const targetUnit = intent.retailUnit
  if (!targetUnit) return { prices: [], excluded: samples.filter(item => item.price != null && item.price > 0).length }
...
function validQuickAmount(value: unknown): value is number {
  return typeof value === 'number' && Number.isFinite(value) && value >= 0
}

function validQuickRate(value: unknown): value is number {
  return validQuickAmount(value) && value <= 100
}

function quickProfitMissing(input: AmazonQuickMarketProfitInput): string[] {
  return [
...
const DECISION_EVIDENCE_FIELDS: Array<[string, string]> = [
...
export function amazonProfitDecisionEvidenceIssues(evidence: AmazonProfitDecisionEvidence = {}): string[] {
 ...
 */
export function buildAmazonQuickMarketProfitFactBlock(intent: AmazonSearchIntent, samples: ClassifiedAmazonMarketSample[], input: AmazonQuickMarketProfitInput = {}, evidence: AmazonProfitDecisionEvidence = {}): string {
...
const FULL_COST_FIELDS: Array<[keyof AmazonFullCostInput, string]> = [
...
function fullCostMissing(input: AmazonFullCostInput): string[] {
  return FULL_COST_FIELDS.flatMap(([field, label]) => {
    const range = input[field]
    if (!validQuickAmount(range?.low) || !validQuickAmount(range?.base) || !validQuickAmount(range?.high)) return [label]
 ...
 */
export function buildAmazonFullCostProfitFactBlock(intent: AmazonSearchIntent, samples: ClassifiedAmazonMarketSample[], quick: AmazonQuickMarketProfitInput = {}, full: AmazonFullCostInput = {}, evidence: AmazonProfitDecisionEvidence = {}): string {
  const direct = samples.filter(item => item.comparisonClass === 'DIRECT')
  const priceSet = normalizedDirectPrices(intent, direct)
  ...
  const scenarios: Array<[string, number, keyof Required<AmazonCostRange>]> = [
    ['悲观', percentile(priceSet.prices, 0.25), 'high' as const],
    ['基准', median(priceSet.prices), 'base' as const],
    ['乐观', percentile(priceSet.prices, 0.75), 'low' as const]
  ]
  ...
  for (const [label, salePrice, rangeKey] of scenarios) {
    ...
    const additional = FULL_COST_FIELDS.reduce((sum, [field]) => sum + (full[field] as Required<AmazonCostRange>)[rangeKey], 0)
...
function entryDecisionNumbers(intent: AmazonSearchIntent, samples: ClassifiedAmazonMarketSample[], quick: AmazonQuickMarketProfitInput, full: AmazonFullCostInput, evidence: AmazonProfitDecisionEvidence) {
  const direct = samples.filter(item => item.comparisonClass === 'DIRECT')
  const priceSet = normalizedDirectPrices(intent, direct)
  if (!intent.retailUnit || !priceSet.prices.length || quickProfitMissing(quick).length || fullCostMissing(full).length || amazonProfitDecisionEvidenceIssues(evidence).length) return null
  const calculate = (salePrice: number, rangeKey: keyof Required<AmazonCostRange>) => {
 ...
 */
export function evaluateAmazonEntryDecision(intent: AmazonSearchIntent, samples: ClassifiedAmazonMarketSample[], audit: AmazonSampleAudit, quick: AmazonQuickMarketProfitInput = {}, full: AmazonFullCostInput = {}, input: AmazonEntryDecisionInput = {}, evidence: AmazonProfitDecisionEvidence = {}): AmazonEntryDecisionResult {
  const evidenceIssues = amazonProfitDecisionEvidenceIssues(evidence)
  const numbers = entryDecisionNumbers(intent, samples, quick, full, evidence)
  if (!numbers) return { decision: '❓ 数据不足，不能判定', reasons: evidenceIssues.length
    ? [`关键经营输入尚未核验：${evidenceIssues.join('、')}。暂缺填零、候选费率和自动提取线索均不得支持“建议入场”。`]
    : ['全成本利润尚不可复算：需具备标准化 DIRECT 售价，以及完整的快速成本和全成本区间。'] }
  if (!meetsAmazonResearchSampleBaseline(audit)) return { decision: '❓ 数据不足，不能判定', reasons: ['研究样本基线未通过，不能据此判断市场竞争力。'], downsideProfit: numbers.downside.profit, baseMargin: numbers.base.margin }
  if (audit.coveragePercent < 80) return { decision: '❓ 数据不足，不能判定', reasons: ['证据覆盖率低于80%，不满足正式“建议入场”证据门槛。'], downsideProfit: numbers.downside.profit, baseMargin: numbers.base.margin }
  if (audit.salesSignalCount < 1) return { decision: '❓ 数据不足，不能判定', reasons: ['未取得 DIRECT 样本的可核验购买信号，无法证明正常动销基础。'], downsideProfit: numbers.downside.profit, baseMargin: numbers.base.margin }
  if (![10, 15, 20, 25].includes(input.targetContributionMargin || Number.NaN)) return { decision: '❓ 数据不足，不能判定', reasons: ['尚未选择公司目标贡献利润率（10%/15%/20%/25%），系统不会自行假设门槛。'], downsideProfit: numbers.downside.profit, baseMargin: numbers.base.margin }
  if (numbers.downside.profit < 0) return { decision: '❌ 不建议入场', reasons: ['悲观情景全成本贡献利润为负，未满足下行现金流硬门禁。'], downsideProfit: numbers.downside.profit, baseMargin: numbers.base.margin }
  if (numbers.base.margin < input.targetContributionMargin!) return { decision: '❌ 不建议入场', reasons: [`基准全成本贡献利润率 ${numbers.base.margin.toFixed(1)}% 低于公司目标 ${input.targetContributionMargin}%。`], downsideProfit: numbers.downside.profit, baseMargin: numbers.base.margin }
  const missing: string[] = []
  if ((input.differentiationEvidence || '').trim().length < 8) missing.push('可核验差异化依据')
  if ((input.complianceIpEvidence || '').trim().length < 8) missing.push('合规/IP核验依据')
  if (missing.length) return { decision: '⚠️ 有条件谨慎入场', reasons: [`全成本与市场样本通过，但入库/下单前必须补齐：${missing.join('、')}。`], downsideProfit: numbers.downside.profit, baseMargin: numbers.base.margin }
...
export function buildAmazonEntryDecisionFactBlock(intent: AmazonSearchIntent, samples: ClassifiedAmazonMarketSample[], audit: AmazonSampleAudit, quick: AmazonQuickMarketProfitInput = {}, full: AmazonFullCostInput = {}, input: AmazonEntryDecisionInput = {}, evidence: AmazonProfitDecisionEvidence = {}): string {
  ...
  if (result.baseMargin != null && result.downsideProfit != null) lines.push(`- 已复算指标：基准全成本贡献利润率 ${result.baseMargin.toFixed(1)}%｜悲观全成本贡献利润 ${money(result.downsideProfit)}。`)
  lines.push('- 报告最终结论必须与系统入场结论完全一致；不得因样本数量、模型评分或销售话术自行上调结论。')
  return lines.join('\n')
}

/** 防止模型把系统门禁结论改写为更乐观的结论。 */
export function validateAmazonEntryDecisionClaim(content: string, expected: AmazonEntryDecision): string[] {
  const decisions: AmazonEntryDecision[] = ['✅ 建议入场', '⚠️ 有条件谨慎入场', '❌ 不建议入场', '❓ 数据不足，不能判定']
  const found = decisions.filter(decision => content.includes(decision))
  if (!found.includes(expected) || found.some(decision => decision !== expected)) return [`系统入场结论为“${expected}”，报告最终结论不得改写为其他等级`]
  return []
}

/** 组装「系统抓取事实块」追加到提问文本：智能体据此填写第二部分大盘与第三部分竞品表（证据等级 事实/系统抓取） */
export function buildMarketFactBlock(keyword: string, samples: AmazonMarketSample[]): string {
  const prices = samples.map(item => item.price).filter((value): value is number => value != null && value > 0)
  const ratings = samples.map(item => item.rating).filter((value): value is number => value != null && value > 0)
  const reviews = samples.map(item => item.reviews).filter((value): value is number => value != null)
  const lines: string[] = []
  ...
  if (ratings.length) lines.push(`- 评分均值：${round1(ratings.reduce((a, b) => a + b, 0) / ratings.length)}（${ratings.length} 个样本）`)
  if (reviews.length) {
...
export function buildComparableMarketFactBlock(intent: AmazonSearchIntent, plan: AmazonKeywordPlan, samples: ClassifiedAmazonMarketSample[], audit: AmazonSampleAudit): string {
  const direct = samples.filter(item => item.comparisonClass === 'DIRECT')
  ...
  const ratings = direct.map(item => item.rating).filter((value): value is number => value != null && value > 0)
  const reviews = direct.map(item => item.reviews).filter((value): value is number => value != null)
  const lines = [
  ...
  if (!researchSampleReady) lines.push('- 研究样本基线未通过：必须同时满足3/3检索词成功、DIRECT不少于15个、DIRECT核心字段覆盖不少于50%、总完整率不少于50%；只能输出补数任务或“数据不足，不能判定”。')
  else lines.push('- 研究样本基线已通过：这只代表可评估市场竞争力，不自动等同于“建议入场”。')
  if (audit.confidence === '低') lines.push('- 样本限制：DIRECT 少于 15 个，只能作为方向性样本，不得写成 TOP50 或完整市场结论。')
  else if (audit.confidence === '中等') lines.push('- 样本限制：DIRECT 为 15–29 个，可作初步判断，但不足以代表完整 TOP50。')
  if (!intent.retailUnit) {
  ...
  if (ratings.length) lines.push(`- DIRECT 评分中位：${round1(median(ratings))}｜均值 ${round1(ratings.reduce((a, b) => a + b, 0) / ratings.length)}（${ratings.length} 个样本）`)
  if (reviews.length) lines.push(`- DIRECT 评论量：中位 ${Math.round(median(reviews)).toLocaleString('en-US')}｜P75 ${Math.round(percentile(reviews, 0.75)).toLocaleString('en-US')}｜均值 ${Math.round(reviews.reduce((a, b) => a + b, 0) / reviews.length).toLocaleString('en-US')}`)
  if (audit.salesSignalCount) lines.push(`- DIRECT 月购买信号：${audit.salesSignalCount}/${direct.length} 个样本在 Amazon 搜索页显示“过去一个月购买量”；可见下限合计 ${audit.salesSignalLowerBound.toLocaleString('en-US')}+。这是页面徽标下限，不是精确月销量，未显示徽标的样本不得按 0 计。`)
...
function isMarketOperationalLine(line: string): boolean {
  if (MARKET_OPERATIONAL_RULE.test(line)) return true
  return /^\|?\s*(?:短期|中期|长期)\s*[（(]/i.test(line)
}

/** 报告后置市场门禁：禁止把不足样本包装成 TOP50、精确销量或无条件 GO。 */
export function validateAmazonMarketClaims(content: string, audit: AmazonSampleAudit): string[] {
  const issues: string[] = []
  const lines = content.split(/\n+/).map(line => line.trim()).filter(Boolean)
  const acceptableUnknown = /待验证|未知|数据不足|未达到|不足以|不能|不可|U\b/i
  const numericPayload = (line: string) => line
    .replace(/TOP\s*50/ig, '')
  ...
  const isTableHeader = (index: number) => lines[index].startsWith('|')
  ...
  if (misleadingTop50) issues.push(`当前系统抓取不是叶子类目完整 TOP50，但报告把 TOP50 写成已验证数值：${misleadingTop50.slice(0, 100)}`)
  const isVerifiedSalesSignal = (line: string) => audit.salesSignalCount > 0
    && /(?:购买徽标|购买信号|过去一个月购买量|bought in past month)/i.test(line)
    && /(?:下限|至少|\+)/.test(line)
    && !/精确月销量|完整市场销量/i.test(line)
  const tableMetricIssue = (() => {
    for (let index = 0; index < lines.length - 1; index += 1) {
      if (!isTableHeader(index)) continue
      ...
      for (let rowIndex = index + 2; rowIndex < lines.length; rowIndex += 1) {
        ...
        if (!row.startsWith('|') || !row.endsWith('|') || isTableDivider(row)) break
  ...
  if (unsupportedMarketMetric) issues.push(`系统未抓取销量、销售额或趋势，但报告输出了无证据数值：${unsupportedMarketMetric.slice(0, 100)}`)
...
/** 从 reviewEvidence 聚合每 ASIN 的高频好评（rating=5）与差评（rating≤2）主题。
 *  纯函数、不调 LLM；按标题去重，长度 < 4 字符的标题丢弃；每 ASIN 最多返回 top3。
 *  用户修改优先由调用方在 profitFieldMeta.origin === '用户修改' 时跳过覆盖。 */
export function buildCompetitorReviewInsights(samples: ClassifiedAmazonMarketSample[], reviewEvidence: AmazonReviewEvidence[], options: { topN?: number; minTitleChars?: number } = {}): string {
  const topN = options.topN ?? 3
  const minChars = options.minTitleChars ?? 4
  const directAsins = new Set(samples.filter(item => item.comparisonClass === 'DIRECT').map(item => item.asin))
  const lines: string[] = []
  lines.push('## 阶段3+ 竞品评论意见聚合（纯聚合，不调大模型）')
  lines.push(`- 口径：仅 DIRECT 竞品的 Amazon 详情页评论样本，按 rating 分桶、标题去重、TOP${topN}；样本不足或缺评论页时输出"待验证"。`)
  if (!reviewEvidence.length || !directAsins.size) {
    lines.push('- 当前未取得 DIRECT 竞品的评论页样本：高频好评/差评均为"待验证"，建议补抓 Amazon 评论页或人工核验。')
    return lines.join('\n')
  }
  const target = reviewEvidence.filter(item => directAsins.has(item.asin)).slice(0, 8)
  if (!target.length) {
    lines.push('- 已抓评论页但无 DIRECT 竞品匹配：高频好评/差评均为"待验证"。')
    return lines.join('\n')
  }
  for (const evidence of target) {
    ...
    for (const snippet of evidence.snippets || []) {
      ...
      if (!title || title.length < minChars) continue
      const key = `${snippet.rating ?? 'na'}|${title}`
      if (seen.has(key)) continue
      seen.add(key)
      if (snippet.rating === 5) positives.push(title)
      else if (snippet.rating != null && snippet.rating <= 2) negatives.push(title)
    }
    const positiveText = positives.length ? positives.slice(0, topN).join('；') : '待验证'
    const negativeText = negatives.length ? negatives.slice(0, topN).join('；') : '待验证'
    lines.push(`- ${evidence.asin}｜高频好评：${positiveText}｜高频差评：${negativeText}｜来源：Amazon 评论页 ${evidence.url}（采买日期 ${evidence.capturedAt.slice(0, 10)}）`)
  }
  return lines.join('\n')
}

/** 从 listingEvidence 拼接每 ASIN 的"核心结构/技术方案"与"配件与包装"摘要。
 *  纯函数、不调 LLM；bulletPoints 缺失时输出"待验证"；超长 bullet 截断 80 字符。 */
export function buildCompetitorListingSummary(samples: ClassifiedAmazonMarketSample[], listingEvidence: AmazonListingEvidence[], options: { maxBullets?: number; maxChars?: number } = {}): string {
  const maxBullets = options.maxBullets ?? 2
  const maxChars = options.maxChars ?? 80
  const directAsins = new Set(samples.filter(item => item.comparisonClass === 'DIRECT').map(item => item.asin))
  const lines: string[] = []
  lines.push('## 阶段3+ 竞品详情页 bullet 摘要（纯聚合，不调大模型）')
  lines.push(`- 口径：仅 DIRECT 竞品的 Amazon 详情页 bullet points，截前 ${maxBullets} 条并限 ${maxChars} 字符；bullet 缺失时输出"待验证"。`)
  if (!listingEvidence.length || !directAsins.size) {
    lines.push('- 当前未取得 DIRECT 竞品的详情页 bullet points：核心结构/配件与包装均为"待验证"。')
    return lines.join('\n')
  }
  const target = listingEvidence.filter(item => directAsins.has(item.asin)).slice(0, 8)
  if (!target.length) {
    lines.push('- 已抓详情页但无 DIRECT 竞品匹配：核心结构/配件与包装均为"待验证"。')
    return lines.join('\n')
  }
  for (const item of target) {
    const bullets = (item.bulletPoints || []).slice(0, maxBullets).map(bullet => String(bullet || '').replace(/\s+/g, ' ').trim().slice(0, maxChars)).filter(Boolean)
    const summary = bullets.length ? bullets.join('；') : '待验证'
    const brand = item.brand ? `｜品牌 ${item.brand}` : ''
    const seller = item.seller ? `｜卖家 ${item.seller}` : ''
    const variant = item.variantSummary ? `｜SKU ${item.variantSummary}` : ''
    lines.push(`- ${item.asin}${brand}${variant}${seller}｜核心结构/技术方案：${summary}｜来源：Amazon 详情页 ${item.url}（采买日期 ${item.capturedAt.slice(0, 10)}）`)
  }
  return lines.join('\n')
}

/** 将系统未抓取的市场数值确定性降级为"待验证"，保留 ASIN、价格、评分与评论量等已抓取字段。 */
export function sanitizeAmazonMarketClaims(content: string, audit?: AmazonSampleAudit): string {
  const lines = content.split('\n')
  const unknown = /待验证|未知|数据不足|未抓取|U\b/i
  const marketMetric = /(?:月销量|月销售额|市场规模|增长趋势|近\s*6\s*月|BSR\/?类目排名|上架时间|广告CPC)/i
  let restrictedColumns: number[] = []
  let inTable = false
