...
export interface ListingLanguageProfile {
  /** 语言代码（en-US/en-GB 区分美式/英式） */
  code: string
  label: string
  /** qwen-mt 翻译模型 target_lang 使用的语言名 */
  mtName: string
}

export const LISTING_LANGUAGES: ListingLanguageProfile[] = [
...
export interface ListingSiteProfile {
  id: string
  platform: string
  site: string
  /** 站点默认语言（勾选站点时自动并入语言集） */
...
export const LISTING_SITES: ListingSiteProfile[] = [
...
export const LISTING_MATERIAL_TERMS: Array<{ zh: string; map: Record<string, string> }> = [
...
export function baseLanguageCode(code: string): string {
  return code.split('-')[0]
}

export function findListingLanguage(code: string): ListingLanguageProfile | undefined {
  return LISTING_LANGUAGES.find(item => item.code === code)
}

/** 翻译引擎术语注入指令：命中素材中出现的术语时，强制按术语表翻译 */
export function buildListingGlossaryDirective(languageCode: string): string {
  const base = baseLanguageCode(languageCode)
  const pairs = LISTING_MATERIAL_TERMS
...
/** 工作台生成查询词（与验收脚本口径一致） */
export function buildListingQuery(platform: string, site: string, languageCode: string, material: string): string {
